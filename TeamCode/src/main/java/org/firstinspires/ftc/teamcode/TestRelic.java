...
@TeleOp(name="TestRelic", group="GG")
@Disabled
public class TestRelic extends OpMode {
   ...
   //static final boolean USE_RELIC = false;
   ...
   Servo servo_relic_arm_;                                 // servo for relic arm
   Servo servo_relic_claw_;                                // servo for relic claw
   double servo_relic_arm_pos_;                            // relic arm servo position
   double servo_relic_claw_pos_;                           // relic claw servo position
   static final double SERVO_RELIC_ARM_INIT = 0.1;         // folded for init
   static final double SERVO_RELIC_ARM_GRAB = 0.76;        // extended for grabing relic
   static final double SERVO_RELIC_ARM_RISE = 1.0;         // rised to pass the wall
   static final double SERVO_RELIC_ARM_DELIVER = SERVO_RELIC_ARM_GRAB+0.05;      // extended for delivering relic
   static final double SERVO_RELIC_CLAW_INIT = 0.3;        // folded for init
   static final double SERVO_RELIC_CLAW_OPEN = 0.0;        // open the claw 
   static final double SERVO_RELIC_CLAW_CLOSE = 0.35;       // close the claw to grab the relic
   ...
   static final double  LEFT_RAMP_DROP = 0.65;             // Hitec2645 digital, limit the power
  ...
  // static final double AUTO_GRAB_TURN_POWER = 0.3;   // max time to turn
   ...
   @Override public void loop () { 
      ...
      if( USE_RAMP ) {
         ...
         telemetry.addData("RampServoPos", String.format("left=%.2f, right=%.2f",servo_left_ramp_.getPosition(),servo_right_ramp_.getPosition()));
      }
      ...
      if( USE_RELIC ) {
         double t = timer_.time();
         double relic_servo_delta = 0.1;
         if( gamepad1.right_trigger>0.5 ) relic_servo_delta=0.02; 
         boolean manual_relic_control = false;
         if( t>last_button_time_+0.3 ) {
            if (gamepad1.dpad_up) {
               servo_relic_arm_pos_ += relic_servo_delta ;
               last_button_time_ = t;
               ...
               servo_relic_claw_pos_ += relic_servo_delta ;
               last_button_time_ = t;
               manual_relic_control = true;
            } else if (gamepad1.dpad_right) {
               servo_relic_claw_pos_ -= relic_servo_delta ;
         ...
         if( rsy > 0.5 ) { // extend the relic slide
            //power_relic_ = RELIC_EXTEND_POWER; 
            power_relic_ = rsy; 
            manual_relic_control = true;
         } else if( rsy<-0.5 ) {  // retract the relic slide
            //power_relic_ = RELIC_RETRACT_POWER; 
            power_relic_ = rsy; 
            manual_relic_control = true;
         }

         if( manual_relic_control ) {
         } else if( a1_cnt_%5==0 ) {
            // no change
         } if( a1_cnt_%5==1 ) {
            // FIXME, ensure the slide is out by 2ft
         ...
         servo_relic_claw_.setPosition( servo_relic_claw_pos_ ); 
         telemetry.addData("Relic: ", "Motore Enc="+String.valueOf(motorRelic_.getCurrentPosition())+", Power="+String.valueOf(motorRelic_.getPower())+"; ArmPos="+String.valueOf(servo_relic_arm_pos_)+", ClawPos="+String.valueOf(servo_relic_claw_pos_)); 
      }
   }
