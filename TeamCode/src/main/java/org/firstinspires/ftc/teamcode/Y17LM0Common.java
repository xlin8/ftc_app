...
//HUB 2 = SIDE HUB
//HUB 3 = BOTTOM HUB
public class Y17LM0Common extends OpMode {
   ...
   /// RELIC
   static final boolean USE_RELIC = true;

   static final double MAX_RELIC_SLIDE_ENC_COUNT = 2200;    //  duct tape on axle to double the diamter, 02/25
   static final double RELIC_EXTEND_POWER = 1.0;
   static final double RELIC_RETRACT_POWER = -RELIC_EXTEND_POWER;

   DcMotor motorRelic_; //port 3 HUB 2 AKA SIDE HUB (motor8, isn't plugged in yet)
   double power_relic_= 0.0;

   Servo servo_relic_arm_; //port____ on hub ____ ...side (2) probably better cuz it has to extend        // servo for relic arm
   Servo servo_relic_claw_; //port____ on hub ____                                // servo for relic claw
   double servo_relic_arm_pos_;                             // relic arm servo position
   double servo_relic_claw_pos_;                         // relic claw servo position
   static final double SERVO_RELIC_ARM_INIT = 0.1;         // folded for init, 2018/03/03
   static final double SERVO_RELIC_ARM_GRAB = 0.58;         // extended for grabing relic, shorter arm, 2018/02/12
   static final double SERVO_RELIC_ARM_RISE = 1.0;         // rised to pass the wall
   static final double SERVO_RELIC_ARM_DELIVER = SERVO_RELIC_ARM_GRAB+0.05;      // extended for delivering relic, 2018/03/02
   static final double SERVO_RELIC_CLAW_INIT = 0.5;        // folded for init
   static final double SERVO_RELIC_CLAW_OPEN = 0.05;        // open the claw
   static final double SERVO_RELIC_CLAW_CLOSE = 0.53;      // close the claw to grab the relic, 2018/02/27
   static final double SERVO_RELIC_CLAW_RELEASE = 0.10;    // open the claw to release the relic, 2018/03/02

   static final double RELIC_RELEASE_SPEED = 0.20;        // how fast to open the relic claw for automatic release, 2018/03/03 
   static final boolean SLOW_LOWER_RELIC = false;        // how fast to open the relic claw for automatic release, 2018/02/27
    ...
    Servo servo_left_winch;  //port 0 on hub 3 BOTTOM HUB
    Servo servo_right_winch; //port 1 on hub 3 BOTTOM HUB
    double servo_left_winch_pos;
    double servo_right_winch_pos;
    static final double SERVO_LEFT_WINCH_LOWER_SPEED1 = 0.0;  // STAGE 1 LOWERING it's basically lifting
    static final double SERVO_RIGHT_WINCH_LOWER_SPEED1 = 1.0;
    static final double SERVO_LEFT_WINCH_LOWER_SPEED2 = 1.0; //STAGE 2 LOWERING
    static final double SERVO_RIGHT_WINCH_LOWER_SPEED2 = 0.0;
   ...
   static final double  MIN_LIFT_POWER = 0.0;
    ...
    Servo servo_front_clamp_;  //port 4 on hub 2                            // front ramp clamp servo
    Servo servo_back_clamp_;   //port 5 on hub 2                            // back ramp clamp servo
    double servo_front_clamp_pos_;                         // front clamp servo position
    double servo_back_clamp_pos_;                          // back clamp servo position

    /////ramp clamp 1
    static final double  FRONT_CLAMP_INIT = 0.36;          // initial position to guide glyph in
    static final double  FRONT_CLAMP_GRAB = 0.08;          // grab the glyph when rising the ramp
    static final double  FRONT_CLAMP_RELEASE = FRONT_CLAMP_INIT;       // release the glyph to let it fall, parallel to the ramp rail
    static final double  BACK_CLAMP_INIT = 0.18;           // FIXXXXX initial position to guide glyph in
    static final double  BACK_CLAMP_GRAB = 0.42;           // FIXXXXX grab the glyph when rising the ramp
    static final double  BACK_CLAMP_RELEASE = BACK_CLAMP_INIT;         // release the glyph to let it fall, parallel to the ramp rail
    ...
    /// RAMP TILT
    static final boolean USE_RAMP = true;                 // use servos to flip ramp

    Servo servo_left_ramp_;     //port 2 on hub 2                           // left intake servo
    Servo servo_right_ramp_;    //port 3 on hub 2                           // right intake servo
    double servo_left_ramp_pos_;                           // Left intake servo position
    double servo_right_ramp_pos_;                          // right intake servo position

    static final double  HITEC_DIGITAL_SERVO_MAX = 0.9;     // Hitec2645 digital, limit the power
    static final double  HITEC_DIGITAL_SERVO_MIN = 1-HITEC_DIGITAL_SERVO_MAX;
    static final double  LEFT_RAMP_INIT = CR_SERVO_STOP;
    static final double  LEFT_RAMP_DROP = 0.75;             // delivering glyphs
    static final double  LEFT_RAMP_RISE = 0.15;             // coming back up or resetting ramp
    ...
    static final double  MAX_RAMP_LIFT_ENC_COUNT = LIFT_ENC_COUNT_PER_GLYPH*2.2;   // 1 glyph => 6in; 2.2x => 13.2in; max 14.5in
    ...
    /// YES DON'T REMOVE USE GLYPH WHEELS = USE INTAKE SYSTEM
    static final boolean USE_GLYPH_WHEELS = true;             // use fly wheels to push/take glyphs

    DcMotor motor_glyph_left_;  //port 0 on hub 2
    DcMotor motor_glyph_right_; //port 1 on hub 2



    /// AUTONOMOUS JEWEL MODULE
    Servo servo_jewel_;   //port 2? on hub 3 BOTTOM HUB?
    Servo servo_knock_;   //port 3? on hub 3 BOTTOM HUB?
    static final double SERVO_JEWEL_INIT = 0.9;
    static final double SERVO_JEWEL_DOWN_POS = 0.35;
    static final double SERVO_KNOCK_INIT = 0.22;                            // jewel module accidentally moved, 2018/02/18
    static final double SERVO_KNOCK_FRONT_POS = SERVO_KNOCK_INIT+0.1;      // front = away from pictograph, back = toward it
    static final double SERVO_KNOCK_BACK_POS = SERVO_KNOCK_INIT-0.1;
    ...
    /////***************************** DELETE *************************************/////
   //INTAKE SERVO
    static final boolean USE_INTAKE_SERVO = false;             // use servo to control fly wheels, USE_GLYPH_WHEELS should be true

    static final double  GLYPH_LEFT_PUSH_POWER = 1.0;         // Hex Motor flipped when mounting on servo
    ...
    // New intake with 9cm wheels
    static final double SERVO_LEFT_INTAKE_INIT = 0.90;        // folded for init
    static final double SERVO_LEFT_INTAKE_GRAB = 0.8;         // grabbing, intake
    static final double SERVO_LEFT_INTAKE_OPEN = 0.65;         // point forward, pusher
    static final double SERVO_LEFT_INTAKE_SHELF = 0.05;       // fold back when shelfing
    static final double SERVO_RIGHT_INTAKE_INIT = 0.10;
    static final double SERVO_RIGHT_INTAKE_GRAB = 0.2;
    static final double SERVO_RIGHT_INTAKE_OPEN = 0.35;
    static final double SERVO_RIGHT_INTAKE_SHELF = 0.95;
    static final boolean USE_HITEC_DIGITAL = false;           // tried Hitec 5645, which broken in 2 runs. 2018/01/28
   static final boolean USE_INTAKE_SERVO_CR = false;          // use CR servo to control intkae wheels to address the high torque, 2018/02/15
    static final double INTAKE_SERVO_CR_REC_TIME = 0.5;       // time for CR servo to recover from FOLD to OPEN/GRAB; disabled if magnetic switch is used
   static final double SERVO_LEFT_INTAKE_INIT_CR = 0.75;        // folded for init
   static final double SERVO_LEFT_INTAKE_GRAB_CR = CR_SERVO_STOP;  // leave servo floating to avoid damage
   static final double SERVO_LEFT_INTAKE_OPEN_CR = CR_SERVO_STOP;  // point forward, pusher; FIXME
   static final double SERVO_LEFT_INTAKE_SHELF_CR = 0.0;  // fold back when shelfing, full power, faster
   static final double SERVO_RIGHT_INTAKE_INIT_CR = 1-SERVO_LEFT_INTAKE_INIT_CR;        // folded for init
   static final double SERVO_RIGHT_INTAKE_GRAB_CR = CR_SERVO_STOP;  // leave servo floating to avoid damage
   static final double SERVO_RIGHT_INTAKE_OPEN_CR = CR_SERVO_STOP;  // point forward, pusher; FIXME
   static final double SERVO_RIGHT_INTAKE_SHELF_CR = 1-SERVO_LEFT_INTAKE_SHELF_CR;  // fold back when shelfing

    //MAGNETIC/TOUCH SENSORS/OTHER INTAKE VARIABLES
    static final boolean USE_INTAKE_MAG_SWITCH = false;       // use Rev magnetic limit switch
    static final boolean USE_INTAKE_MAG_HOLD_POS = false;     // use Rev magnetic limit switch to hold position
    DigitalChannel mag_intake_left_;
   DigitalChannel mag_intake_right_;
    static final boolean USE_INTAKE_TOUCH = false;            // use Rev touch sensor
    DigitalChannel touch_intake_left_;
   DigitalChannel touch_intake_right_;
   static final int INTAKE_INIT = 0; 
   static final int INTAKE_PUSH = 1; 
   static final int INTAKE_PULL = 2; 
   static final int INTAKE_SHELF = 3; 
   int intake_curr_state_ = INTAKE_INIT ;
   int intake_prev_state_ = INTAKE_INIT ;
   ...
   @Override public void init() {
       ...
       if( USE_RAMP ) {
           servo_left_ramp_ = hardwareMap.servo.get("servo_left_ramp");   //port 4, left when looking from dumping side
           servo_right_ramp_ = hardwareMap.servo.get("servo_right_ramp");  // port 5
       }
       ...
       if(USE_LIFT) {
           motorLift_ = hardwareMap.dcMotor.get("motor7");
      ...
      if( USE_INTAKE_MAG_SWITCH ) {
         mag_intake_left_ =  hardwareMap.get(DigitalChannel.class, "mag_intake_left");  // must use DigitalChannel for REV magetic switch
         mag_intake_left_.setMode(DigitalChannelController.Mode.INPUT);
         mag_intake_right_ =  hardwareMap.get(DigitalChannel.class, "mag_intake_right");  // must use DigitalChannel for REV magnetic switch
         mag_intake_right_.setMode(DigitalChannelController.Mode.INPUT);
      }
      if( USE_INTAKE_TOUCH ) {
         touch_intake_left_ =  hardwareMap.get(DigitalChannel.class, "touch_intake_left");  // must use DigitalChannel for REV touch sensor
         touch_intake_left_.setMode(DigitalChannelController.Mode.INPUT);
         touch_intake_right_ =  hardwareMap.get(DigitalChannel.class, "touch_intake_right");  // must use DigitalChannel for REV touch sensor
         touch_intake_right_.setMode(DigitalChannelController.Mode.INPUT);
      }


      if( USE_INTAKE_SERVO ) {  // close bottom jaw to avoid conflict with intake motor
